...
if [ $FORMAT = "mp4" ]; then
    ...
    ffmpeg -hide_banner -v 16 -r 24 -pattern_type glob -i "/input/$PATTERN-$DATE_FORMATTED-*.jpg" -vf scale=$RESOLUTION1:$RESOLUTION2 -vcodec libx264 -y /output/$DATE_FORMATTED.$FORMAT
    echo "Done...."
    ...
    ffmpeg -hide_banner -v 16 -i /tmp/$DATE_FORMATTED.mp4 -vf "fps=10,scale=$RESOLUTION1:$RESOLUTION2:flags=lanczos,palettegen" -y /tmp/palette.png
    ffmpeg -hide_banner -v 16 -i /tmp/$DATE_FORMATTED.mp4 -i /tmp/palette.png -lavfi "fps=10,scale=$RESOLUTION1:$RESOLUTION2:flags=lanczos [x]; [x][1:v] paletteuse" -y /output/$DATE_FORMATTED.$FORMAT
